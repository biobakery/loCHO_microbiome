...
High_protein_assocations <-read.table('HALLA/RES_FILT/Protein_res/all_associations.txt', header=T, sep = "\t", quote="",
                                   comment.char="")
...
merged_data <- full_join(High_protein_assocations,High_fat_assocations,by=c("X_features","Y_features"))
  

#things that were in one dataset not the other get correlations of 0
...
merged_data <- merged_data %>% mutate(absdiff=abs(association.x-association.y)) %>% mutate(sign=association.x*association.y) %>%
...
saveRDS(merged_data, "HALLA/RES_FILT/all_loCHO_merged_res.RDS")
...
final_plot
ggsave(filename = "HALLA/RES_FILT/Cor_diffs_lowCHO.pdf",
       height=6, width=12)


## Compare density of abs

merged_data_fat <- readRDS("HALLA/RES_FILT/Fat_res/Fat_merged_res.RDS")
merged_data_protein <- readRDS("HALLA/RES_FILT/Protein_res/protein_merged_res.RDS")
merged_data_all_LOCHO <- merged_data


merged_data_fat$Diet_Comp <- "High Carb - High Fat"
merged_data_protein$Diet_Comp <- "High Carb - High Protein"
merged_data_all_LOCHO$Diet_Comp <- "High Protein - High Fat"

merged_data_fat <- merged_data_fat %>% select(Diet_Comp, absdiff)
merged_data_protein <- merged_data_protein %>% select(Diet_Comp, absdiff)
merged_data_all_LOCHO <- merged_data_all_LOCHO %>% select(Diet_Comp, absdiff)

merged_total <- rbind(merged_data_fat, merged_data_protein)
merged_total <- rbind(merged_total, merged_data_all_LOCHO)

#significantly different as well
wilcox.test(absdiff ~ Diet_Comp, data=merged_total)

density_all <- merged_total %>% ggplot(aes(x=absdiff, fill=Diet_Comp)) + geom_density(alpha=0.5) +
  theme_classic() +
  xlab("Absolute difference in correlation")
density_all

ggsave("HALLA/Corr_diff_Denisty_all.pdf", plot=density_all, height=4, width=6)
