...
High_protein_assocations <-read.table('HALLA/RES_FILT/Protein_res/all_associations.txt', header=T, sep = "\t",
                                  quote="", comment.char = "")


#join the datasets and get absolute difference and signs
merged_data <- full_join(High_carb_assocations,High_protein_assocations,by=c("X_features","Y_features"))
...
ggsave(filename = "HALLA/RES_FILT/multi_omics_overview_High_protein_all.pdf",
       height=6, width=12)

saveRDS(merged_data, file="HALLA/RES_FILT/Protein_res/protein_merged_res.RDS")
...
keep_metas <- merged_data_sig_effect_filt %>% group_by(Y_features) %>% summarize(number=n()) %>% filter(number > 3) %>% pull(Y_features)
...
ggsave(filename = "HALLA/RES_FILT/multi_omics_overview_High_protein.pdf",
       height=6, width=12)
